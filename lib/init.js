  ...
  return write(rc, '{}').then(() => {
    if (options._passive) {
      if (options.env === 'production') {
        throw new Error('Honeypack: Missing webpack configuration file, this is a fatal error in production.');
      }
      process.stdout.write(chalk.yellow(`Honeypack: Missing webpack configuration file, start initialization\n`));
    }
