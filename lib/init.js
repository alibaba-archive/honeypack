...
const {promiseChain, camelCase, getPackageManager} = require('./helpers');
...
class InitGenerator extends Generator {
  ...
  prompting() {
    ...
    return promiseChain([
      ...
      const fragments = Object.keys(deps)
        ...
        .map(key => `const ${deps[key] === REQUIRE_WITH_CAMEL_CASE ? camelCase(key) : key} = require('${key}');`);

      fragments.push(transform(config));

      return write(this.configFile, prettier.format(fragments.join('\n'), {parser: 'babylon', singleQuote: true}));
  ...
  _errorHandler(err, prefix = 'Honeypack Internal') {
