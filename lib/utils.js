...
module.exports = {
  ...
  print(config) {
    ...
    console.log('======= honeypack output start =======');
    console.log('============== loaders ===============');
    module.rules.forEach((rule, index) => {
      console.log(`  test: ${rule.test} \n   use: ${rule.use.loader || rule.use.map(use => use.loader || use)}`);
      module.rules.length > index + 1 &&
      console.log('--------------------------------------');
    });
    console.log('============== plugins ===============');
    plugins.forEach((plugin, index) => {
      console.log('plugin:', plugin.constructor.name);
      plugins.length > index + 1 &&
      console.log('--------------------------------------');
    });
    console.log('======= honeypack output end =========');
  }
};
